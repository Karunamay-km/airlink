...
public class SeatController {
    ...
    @ApiResponse(
            ...
            content = @Content(schema = @Schema(implementation = BaseSeatResponseDTO.class))
    )
    @ApiResponse(responseCode = "400", description = "Invalid input data or validation failure")
    @ApiResponse(responseCode = "404", description = "Flight not found")
    @ApiResponse(responseCode = "409", description = "Seat number already exists on this flight")
    @PostMapping
    ...
    public ResponseEntity<RestApiResponse<SeatResponseDTO>> createSeat(
        ...
        log.info("REST: Create seat request (SeatNo: {}, FlightId: {})",
    ...
    @ApiResponse(
            ...
            content = @Content(schema = @Schema(implementation = PaginationSeatResponseDTO.class))
    )
    @GetMapping("/available")
    public ResponseEntity<RestApiResponse<PageResponseDTO<SeatResponseDTO>>> getAvailableSeats(
        ...
        return ResponseEntity.ok(
                RestApiResponse.success(seatService.getSeatByAvailableTrue(pageable))
        );
    }

    @Operation(
            summary = "Get all seats for a flight",
            description = "Retrieves a paginated list of seats for a given flight number."
    )
    @ApiResponse(
            responseCode = "200",
    ...
    public ResponseEntity<RestApiResponse<PageResponseDTO<SeatResponseDTO>>> getSeatsByFlightNo(
            @Parameter(description = "Flight number (e.g., AI-101)") @RequestParam String flightNo,
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "20") int size,
            @RequestParam(defaultValue = "id") String sortBy,
            @RequestParam(defaultValue = "ASC") Sort.Direction direction
        ...
        Pageable pageable = PageRequest.of(page, size, direction, sortBy);
        return ResponseEntity.ok(
                RestApiResponse.success(seatService.getAllByFlightNo(flightNo, pageable))
        );
    }
    ...
    public ResponseEntity<RestApiResponse<SeatResponseDTO>> getSeatBySeatNo(
            @Parameter(description = "Seat number (e.g., 12A)") @RequestParam String seatNo
    ) {
    ...
    public ResponseEntity<RestApiResponse<Integer>> getAvailableSeatCountByFlight(
        ...
        return ResponseEntity.ok(
                RestApiResponse.success(seatService.getAvailableSeatCountByFlight(flightId))
        );
    }
    ...
    @ApiResponse(responseCode = "404", description = "Seat not found")
    @ApiResponse(responseCode = "409", description = "Seat has existing bookings and cannot be deleted")
    @DeleteMapping("/{id}")
