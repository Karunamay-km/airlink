...
import com.karunamay.airlink.dto.user.AuthenticationResponseDTO;
import com.karunamay.airlink.dto.user.UserLoginRequestDTO;
...
public class AuthenticationController {
    ...
    public ResponseEntity<RestApiResponse<AuthenticationResponseDTO>> registerUser(
            @Valid @RequestBody UserRegistrationRequestDTO request) {
        log.info("REST: Register user request received for username: {}", request.getUsername());
        AuthenticationResponseDTO user = userService.registerUser(request);
        return ResponseEntity
                .status(HttpStatus.CREATED)
                .body(RestApiResponse.success("User registered successfully", user));
    }

    @Operation(
            summary = "User login",
            description = "Authenticates user credentials and issues new JWT access and refresh tokens via **HTTP-only cookies** and in the response body.",
            tags = {"Authentication"}
    )
    @ApiResponses(value = {
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
    ...
    public ResponseEntity<RestApiResponse<AuthenticationResponseDTO>> login(
        ...
        log.info("REST: Login user request received for email: {}", request.getEmail());
        AuthenticationResponseDTO authenticateUser = userService.authenticateUser(request);
        ResponseCookie accessTokenCookie = ResponseCookie.from("accessToken", authenticateUser.getAccessToken())
                .httpOnly(true)
                .secure(true)
                .path("/")
                .maxAge(18000L)
        ...
        ResponseCookie refreshTokenCookie = ResponseCookie
                .from("refreshToken", authenticateUser.getRefreshToken())
                .httpOnly(true)
                .secure(true)
                .path("/")
                .maxAge(3600000L)
        ...
        return ResponseEntity.ok()
                ...
                .body(RestApiResponse.success("Login successful", authenticateUser));
    }
    ...
    @GetMapping("/refresh")
    public ResponseEntity<RestApiResponse<AuthenticationResponseDTO>> refreshToken(
        ...
        AuthenticationResponseDTO authResponse = userService.refreshToken(token);
        ...
        return ResponseEntity.ok()
                .header(HttpHeaders.SET_COOKIE, accessTokenCookie.toString())
                .body(RestApiResponse.success("Token refreshed successfully", authResponse));
    }
    ...
    public ResponseEntity<RestApiResponse<Void>> forgotPassword(
        ...
        return ResponseEntity.ok(RestApiResponse.success("Password reset instruction sent to email", null));
    }
